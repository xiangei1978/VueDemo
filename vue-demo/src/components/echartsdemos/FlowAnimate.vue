  ...
  mounted() {
    this.$watch(
      ...
      function(newVal, oldVal) {
        ...
        if (this.loadingIndexNum == 0) {
          for (let i = 0; i < this.loadingArray.length; i++) {
            if (this.loadingArray[i][1] != this.loadingArray[0][1]) {
              delnum = i;
              break;
            }
            this.loadingIndexNum++;

            let tempi = this.loadingArray[i][0].replace("myChart", "");
            console.log(
    ...
    //根据权重排序
    this.loadingArray.sort(function(a, b) {
      return a[1] - b[1];
    });
    console.log("loadingArray = " + this.loadingArray);
